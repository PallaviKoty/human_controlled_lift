...
void GazeboRosP3D::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (this->topic_name_ != "")
  {
    ...
    this->pub_string = this->rosnode_->advertise<std_msgs::String>("/elevator", 10);
  }
  ...
  if (this->pub_.getNumSubscribers() > 0)
  {
    ...
    if (tmp_dt != 0)
    {
      ...
      if (this->topic_name_ != "")
      {
        ...
        if((this->pose_msg_.pose.pose.position.x >=2.0) && (this->pose_msg_.pose.pose.position.x < 3.0) && (this->pose_msg_.pose.pose.position.y >= -1.25) && (this->pose_msg_.pose.pose.position.y < 1.25))
        {
          if((this->pose_msg_.pose.pose.position.z >=0.0) && (this->pose_msg_.pose.pose.position.z < 0.75))
          {
            msg.data = '0';  
          }
          else if ((this->pose_msg_.pose.pose.position.z >=3.0) && (this->pose_msg_.pose.pose.position.z < 3.75))
          {
            msg.data = '1';
          }
          else if ((this->pose_msg_.pose.pose.position.z >=6.0) && (this->pose_msg_.pose.pose.position.z < 6.75))
          {
            msg.data = '2';
          }
          // ROS_INFO("%s", msg.data.c_str());
          (this->pub_string).publish(msg);
        }
        // if((this->pose_msg_.pose.pose.position.y >=0.0) && (this->pose_msg_.pose.pose.position.y < 0.5))
        // {
        //   msg.data = '0';
        //   ROS_INFO("%s", msg.data.c_str());
        //   (this->pub_string).publish(msg);
        // }
        // if((this->pose_msg_.pose.pose.position.z >=0.0) && (this->pose_msg_.pose.pose.position.z < 0.5))
        // {
        //   msg.data = '0';
        //   ROS_INFO("%s", msg.data.c_str());
        //   (this->pub_string).publish(msg);
